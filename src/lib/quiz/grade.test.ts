...
test('multi_select is order-independent but exact-set', () => {
  assert.equal(
    gradeQuiz(questions, { q2: [0, 3] }).perQuestion.q2.correct,
    true,
  );
  assert.equal(
    gradeQuiz(questions, { q2: [3, 0] }).perQuestion.q2.correct,
    true,
  );
  assert.equal(gradeQuiz(questions, { q2: [0] }).perQuestion.q2.correct, false); // subset
  assert.equal(
    gradeQuiz(questions, { q2: [0, 1, 3] }).perQuestion.q2.correct,
    false,
  ); // superset
});
...
test('numeric respects tolerance band', () => {
  assert.equal(
    gradeQuiz(questions, { q3: '10.5' }).perQuestion.q3.correct,
    true,
  ); // edge
  assert.equal(
    gradeQuiz(questions, { q3: '9.5' }).perQuestion.q3.correct,
    true,
  );
  assert.equal(
    gradeQuiz(questions, { q3: '10.6' }).perQuestion.q3.correct,
    false,
  );
  assert.equal(
    gradeQuiz(questions, { q3: 'abc' }).perQuestion.q3.correct,
    false,
  );
});
...
test('missing / wrong-type answers score 0, never throw', () => {
  ...
  assert.equal(
    gradeQuiz(questions, { q1: [1, 2] }).perQuestion.q1.correct,
    false,
  ); // array for MC
});
