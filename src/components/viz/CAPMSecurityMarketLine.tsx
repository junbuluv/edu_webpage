...
const initialAssets: Asset[] = [
  ...
  { name: 'S&P 500',      beta: 1.00, observed: 0.105 },
  { name: 'Tech ETF',     beta: 1.30, observed: 0.150 },
  { name: 'Levered HF',   beta: 1.80, observed: 0.140 },
];
  ...
  return (
    <div className="my-8 rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap gap-6">
        <Slider label="Risk-free rate Rf" v={s.rf} min={0} max={0.10} step={0.0025}
          fmt={(v) => (v * 100).toFixed(2) + '%'}
          onChange={(v) => setS((x) => ({ ...x, rf: v }))} />
        <Slider label="Market risk premium" v={s.mrp} min={0.02} max={0.15} step={0.005}
        ...
        <tbody>
          {points.map((p) => (
            <tr key={p.name} className="border-t border-slate-100">
              ...
              <td className={p.alpha >= 0 ? 'text-emerald-700' : 'text-rose-700'}>
                {p.alpha >= 0 ? '+' : ''}{(p.alpha * 100).toFixed(1)}%
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <p className="mt-3 text-xs text-ink-muted">
        ...
        Below the line ⇒ negative α. In a CAPM-efficient market, α = 0 for
        every asset on average.
      </p>
    </div>
  );
}
