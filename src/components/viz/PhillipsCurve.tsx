  ...
  return (
    <div className="my-8 rounded-lg border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap gap-6">
        <Slider label="Expected inflation πᵉ (%)" v={state.piE} min={0} max={10} step={0.1}
          onChange={(v) => setState((s) => ({ ...s, piE: v }))} />
        <Slider label="Natural rate uₙ (%)" v={state.un} min={2} max={8} step={0.1}
          onChange={(v) => setState((s) => ({ ...s, un: v }))} />
        <Slider label="Slope β" v={state.beta} min={0.1} max={1.5} step={0.05}
          onChange={(v) => setState((s) => ({ ...s, beta: v }))} />
        <Slider label="Actual unemployment u (%)" v={state.u} min={0} max={12} step={0.1}
      ...
      <div className="mt-4 h-72">
        <ResponsiveContainer>
          <LineChart data={data}>
            ...
            <XAxis dataKey="u" tickFormatter={(v) => v.toFixed(0)}
            ...
            <Line type="monotone" dataKey="pi" name="Phillips curve" stroke="#2563eb" dot={false} />
            <ReferenceDot x={state.u} y={today} r={5} fill="#0f172a" stroke="white" />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
