-- Pre-workshop-integrity schema snapshot, used by the CI upgrade test.
-- Captured from commit 6e5ef3073b0813b6136360fb69296d198e69ee72, which
-- lived on the workshop-integrity branch and was orphaned when that
-- branch was rebased before merging (#109). CI previously fetched it
-- with 'git show <sha>', which broke the moment the commit became
-- unreachable. It is vendored here so the upgrade path is reproducible
-- from the working tree alone, with no dependency on git history.
--
-- This is a historical snapshot: do NOT update it to match the current
-- schema. Its whole purpose is to be the 'before' state that
-- supabase/schema.sql and the migrations are applied on top of.
